...
class GWMMetaModel:
    ...
    def __init__(self, config):
        super(GWMMetaModel, self).__init__(config)
...
class GWMMetaForCausalLM(ABC):
    ...
    def encode_graphs(self, graph, graph_emb):
        embedding_list=[]
        inter_num=0
        for inter in range(graph_emb.shape[1]):
            tem_features=graph_emb[:,inter,:]
            tem_embedding = self.get_model().mm_projector[inter_num](tem_features)
            tem_embedding = self.get_model().mm_projector[inter_num+1](tem_embedding)
            tem_embedding=nn.GELU()(tem_embedding)
            # tem_embedding = self.get_model().mm_projector[inter](tem_features)
            embedding_list.append(tem_embedding.unsqueeze(1))
            inter_num+=2
        embedding_list=torch.cat(embedding_list,dim=1)
